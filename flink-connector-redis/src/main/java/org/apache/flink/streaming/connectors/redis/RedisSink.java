...
public class RedisSink<IN> extends RichSinkFunction<IN> {
     ...
     */
    public RedisSink(FlinkJedisConfigBase flinkJedisConfigBase, RedisMapper<IN> redisSinkMapper, DynamicTableSink.DataStructureConverter converter, boolean isAppendOnly) {
    ...
    public void invoke(IN input, Context context) throws Exception {
        Row row = (Row) this.converter.toExternal(input);
        Tuple2<Boolean, Row> rowTuple2 = new Tuple2<Boolean, Row>(this.isAppendOnly, row);

        String key = redisSinkMapper.getKeyFromData((IN) rowTuple2);
        String value = redisSinkMapper.getValueFromData((IN) rowTuple2);

        Optional<String> optAdditionalKey = redisSinkMapper.getAdditionalKey((IN) rowTuple2);
        Optional<Integer> optAdditionalTTL = redisSinkMapper.getAdditionalTTL((IN) rowTuple2);
